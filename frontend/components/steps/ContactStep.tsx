...
import { PhoneField, PostalCodeField } from '@/components/form/controlled';
...
export default function ContactStep({ slice, onSaveAndContinue, onBack, isFirst, isLast, saving }: StepProps) {
  ...
  const canContinue = contactSchema.safeParse(watch()).success;
    ...
    <form onSubmit={handleSubmit((d) => onSaveAndContinue(d))} noValidate>
      <StepShell title="Contact details" description="How we reach you, and where you live.">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <SelectField label="City" required placeholder="Select city"
            options={MAJOR_CITIES.map((c) => ({ value: c, label: c }))}
            {...register('city')} error={e.city?.message} />
          <SelectField label="Province" required placeholder="Select province"
            options={PROVINCES.map((p) => ({ value: p, label: p }))}
            {...register('province')} error={e.province?.message} />
          <PostalCodeField control={control} name="postalCode" />
        </div>
      </StepShell>
